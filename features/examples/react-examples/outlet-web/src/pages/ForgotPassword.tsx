...
import { Mail, ArrowRight, ArrowLeft, Loader2, ShoppingBag, ShieldCheck, AlertCircle } from 'lucide-react';
...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage('Security recovery signal dispatched to your registry email.');
      setTimeout(() => {
        navigate(`/reset-password?email=${encodeURIComponent(email)}`);
      }, 2000);
    } catch (err: any) {
      setError(err.response?.data?.error || 'Recovery request failed. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Access Recovery Branch</p>
        </div>
